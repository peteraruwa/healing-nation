...
export default function EventsSection() {
  ...
  return (
    <section style={{ background: '#2c2925', color: '#f4f3ea', padding: 'clamp(40px,6vw,76px) 0' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '0 clamp(16px,5vw,40px)', display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 20, marginBottom: 'clamp(26px,4vw,40px)' }}>
        <div className="hn-reveal" style={{ borderLeft: '4px solid var(--accent,#d6a96f)', paddingLeft: 18 }}>
          <div style={{ color: 'var(--accent,#d6a96f)', fontSize: 12.5, fontWeight: 800, letterSpacing: '.22em' }}>WHERE WE'RE GOING NEXT</div>
          <h2 style={{ fontFamily: "'Lora',serif", fontWeight: 700, fontSize: 'clamp(28px,4.5vw,46px)', margin: '4px 0 0', color: '#fff' }}>Upcoming Events</h2>
        </div>
        <span className="hn-hide-mobile" style={{ fontSize: 13, color: '#9a9183', fontWeight: 600 }}>Scroll for more &rarr;</span>
      </div>

      <div className="hn-marquee" style={{ overflow: 'hidden', maskImage: 'linear-gradient(90deg,transparent,#000 6%,#000 94%,transparent)' }}>
        <div className="hn-marquee-track hn-marquee-track-events" style={{ display: 'flex', width: 'max-content' }}>
          {loop.map((ev, i) => (
            <article key={ev.title + ev.location + i} className="hn-ticket" style={{ flex: 'none', width: 'clamp(300px,30vw,360px)', display: 'flex', margin: '0 10px' }}>
              <div style={{ flex: 'none', width: 78, background: 'var(--accent,#a9783f)', color: '#fff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '18px 6px' }}>
              ...
              <div style={{ flex: 1, background: '#3a352f', borderLeft: '1px dashed rgba(255,255,255,.25)', padding: '18px 20px', display: 'flex', flexDirection: 'column' }}>
                <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.1em', color: 'var(--accent,#d6a96f)', marginBottom: 6 }}>{ev.tag.toUpperCase()}</div>
