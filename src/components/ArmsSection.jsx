    ...
    <section style={{ background: '#eceae4', padding: 'clamp(40px,6vw,80px) clamp(16px,5vw,40px)' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto' }}>
        <div className="hn-reveal" style={{ textAlign: 'center', marginBottom: 'clamp(26px,4vw,44px)' }}>
          <div style={{ color: 'var(--accent-dark,#875f31)', fontSize: 13, fontWeight: 800, letterSpacing: '.2em' }}>HOW WE CARRY THE MANDATE</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(260px,1fr))', gap: 'clamp(16px,2.5vw,26px)' }}>
          {arms.map((a, i) => (
            <div key={a.num} className="hn-reveal hn-arm-card" style={{ background: '#fff', borderRadius: 8, padding: 'clamp(26px,3vw,36px)', boxShadow: '0 10px 28px rgba(0,0,0,.07)', borderTop: '4px solid var(--accent,#a9783f)', display: 'flex', flexDirection: 'column', animationDelay: `${i * 0.12}s` }}>
