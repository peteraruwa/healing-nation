...
export default function Header({ isDesktop, menuOpen, toggleMenu, closeMenu }) {
  return (
    <header style={{ background: '#fff', position: 'sticky', top: 0, zIndex: 50, boxShadow: '0 1px 0 rgba(0,0,0,.06)' }}>
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '0 clamp(16px,5vw,40px)', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 20 }}>
        <a href="#main" aria-label="Healing Nations Global Outreach — home" style={{ alignSelf: 'flex-start', display: 'flex', alignItems: 'center', gap: 12, background: 'var(--accent,#a9783f)', color: '#fbf6ee', padding: '11px clamp(18px,3vw,28px) 13px', borderRadius: '0 0 12px 12px', textAlign: 'left', boxShadow: '0 6px 14px rgba(0,0,0,.12)' }}>
          <picture>
        ...
        {isDesktop ? (
          <nav aria-label="Primary" style={{ display: 'flex', alignItems: 'center', gap: 'clamp(16px,2.2vw,30px)', padding: '22px 0 0', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            {navLinks.map((n) => (
          ...
          <button
            ...
            style={{ marginTop: 16, width: 46, height: 46, border: 'none', borderRadius: 8, background: 'var(--accent,#a9783f)', display: 'grid', placeItems: 'center', gap: 4, cursor: 'pointer', padding: 0 }}
          >
            <span className="hn-hamburger-bar" style={{ display: 'block', width: 20, height: 2.4, background: '#fff', borderRadius: 2 }}></span>
            <span className="hn-hamburger-bar" style={{ display: 'block', width: 20, height: 2.4, background: '#fff', borderRadius: 2 }}></span>
            <span className="hn-hamburger-bar" style={{ display: 'block', width: 20, height: 2.4, background: '#fff', borderRadius: 2 }}></span>
          </button>
        )}
      ...
      {!isDesktop && menuOpen && (
        <nav id="hn-mobile-nav" aria-label="Primary" className="hn-mobile-menu" style={{ background: '#fff', borderTop: '1px solid #eee', padding: '8px clamp(16px,5vw,40px) 18px' }}>
          {navLinks.map((n) => (
